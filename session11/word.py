...
def find_words_no_e():
    ...
    number_of_words_no_e = 0
    number_of_words = 0
    for line in f:
        word = line.strip()
        # increment total number of words
        number_of_words += 1
        # if the word contains no e
        #    increment the counting variable
        if has_no_e(word): # same as if has_no_e(word) == True:
            number_of_words_no_e += 1
        
    return number_of_words_no_e/number_of_words
